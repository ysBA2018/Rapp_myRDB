...
class ChangeRequestsSerializer(serializers.HyperlinkedModelSerializer):
    class Meta:
        ...
        fields = (
            'url', 'pk', 'requesting_user', 'requesting_user_hat_userid_name_combination',
            'compare_user', 'compare_user_hat_userid_name_combination', 'action', 'right_name',
            'right_type', 'reason_for_action', 'created', 'last_modified', 'status', 'reason_for_decline')

    url = serializers.HyperlinkedIdentityField(view_name='myRDBNS:changerequests-detail')
    requesting_user_hat_userid_name_combination = serializers.HyperlinkedRelatedField(read_only=True, view_name='myRDBNS:userhatuseridundname-detail')
    compare_user_hat_userid_name_combination = serializers.HyperlinkedRelatedField(read_only=True, view_name='myRDBNS:userhatuseridundname-detail')
...
class UserHatTblUserIDundNameSerializer(serializers.HyperlinkedModelSerializer):
    class Meta:
        ...
        fields = ('url', 'id', 'user_name', 'userid_name_id', 'rollen', 'delete_list', 'transfer_list', 'my_requests')
    ...
    my_requests = serializers.HyperlinkedRelatedField(many=True, read_only=True, view_name='myRDBNS:changerequests-detail')
    ...
    def set_rights_as_requested(self, instance, data):
        ...
        for obj in objects_to_change:
            ...
            if action == 'apply':
                list = instance.transfer_list.all()
            else:
                list = instance.delete_list.all()
            if right_type == 'ROLLE':
                for rolle in list:
    ...
    def restore_after_decline(self, instance, request_data, action):
        ...
        if request_data['right_type'] == 'ROLLE':
            data = {'right_name': request_data['right_name'], 'right_type': request_data['right_type'].lower()}

        elif request_data['right_type'] == 'AF':
            for rolle in list:
                for af in rolle.applied_afs.all():
                    if af.model_af_id.af_name == request_data['right_name']:
                        data = {'right_name': af.model_af_id.af_name, 'parent': rolle.model_rolle_id.rollenname,
            ...
            for rolle in list:
                for af in rolle.applied_afs.all():
                    for gf in af.applied_gfs.all():
                        if gf.model_gf_id.name_gf_neu == request_data['right_name']:
                            data = {'right_name': gf.model_gf_id.name_gf_neu, 'parent': af.model_af_id.af_name, 'grandparent': rolle.model_rolle_id.rollenname,
                                    'right_type': request_data['right_type'].lower()}
                            break
                    else:
                        continue
    ...
    def apply_right(self, instance, compare_xv_user, right_name, right_type, application_date):
        ...
        if right_type == "ROLLE":
            for rolle in instance.transfer_list.all():
                if rolle.model_rolle_id.rollenname == right_name:
                    #af.af_applied = application_date
                    instance.rollen.add(rolle.id)
                    #instance.direct_connect_afs_id.add(af.model_af_pk)
                    instance.transfer_list.remove(rolle)
                    break
        elif right_type == "AF":
            for rolle in instance.transfer_list.all():
                for af in rolle.applied_afs.all():
                    if af.model_af_id.af_name == right_name:
                        for u_rolle in instance.rollen.all():
                            if u_rolle.model_rolle_id.rollenname == rolle.model_rolle_id.rollenname:
                                u_rolle.applied_afs.add(af.id)
                                rolle.applied_afs.remove(af)
                                break
            ...
            for rolle in instance.transfer_list.all():
                for af in rolle.applied_afs.all():
                    for gf in af.applied_gfs.all():
                        if gf.model_gf_id.name_gf_neu == right_name:
                            for u_rolle in instance.rollen.all():
                                if u_rolle.model_rolle_id.rollenname == rolle.model_rolle_id.rollenname:
                                    for u_af in u_rolle.applied_afs.all():
                                        if u_af.model_af_id.af_name == af.model_af_id.af_name:
                                            u_af.applied_gfs.add(gf.id)
                                            af.applied_gfs.remove(gf)
                                            break
                    if not af.applied_gfs.all():
                        rolle.applied_afs.remove(af)
                        break
                if not rolle.applied_afs.all():
                    instance.transfer_list.remove(rolle)
                            ...
                            if tf.model_tf_id.tf == right_name.lower():
                                for u_rolle in instance.rollen.all():
                                    if u_rolle.model_rolle_id.rollenname == rolle.model_rolle_id.rollenname:
                                        for u_af in u_rolle.applied_afs.all():
                                            if u_af.model_af_id.af_name == af.model_af_id.af_name:
                                                for u_gf in u_af.applied_gfs.all():
                                                    if u_gf.model_gf_id.name_gf_neu == gf.model_gf_id.name_gf_neu:
                                                        u_gf.applied_tfs.add(tf.id)
    ...
    def delete_right(self, instance, right_name, right_type):
        ...
        if right_type == "ROLLE":
            for rolle in instance.delete_list.all():
                if rolle.model_rolle_id.rollenname == right_name:
                    instance.delete_list.remove(rolle)
                    break
        elif right_type == "AF":
            for rolle in instance.delete_list.all():
                for af in rolle.applied_afs.all():
    ...
    def remove_from_my_requests(self, instance, data):
        ...
        instance.my_requests.remove(int(data['request_pk']))
        return instance
    ...
    def add_to_transfer_list(self, instance, data):
        ...
        upper_compare_user_name = data['compare_user'].upper()
        compareUser = TblUserIDundName.objects.get(userid=upper_compare_user_name)
        compareUserRights = UserHatTblUserIDundName.objects.get(userid_name_id=compareUser.id)
        if data['right_type'] == 'role':
            for rolle in compareUserRights.rollen.all():
                if rolle.model_rolle_id.rollenname == data['right_name']:
                    ...
                    for trans_rolle in instance.transfer_list.all():
                        if rolle.model_rolle_id.rollenid == trans_rolle.model_rolle_id.rollenid:
                            ...
                            break
                    if not rolle_found:
                        '''TODO: FIX LÖSCHEN VON NESTED-MODELS bei remove von Transferliste
                                |--> CLONE von Model erzeugen mit anderen ID'S auf allen Hierarchieebenen'''
                        cpy_rolle = TblAppliedRolle.objects.create(model_rolle_id=rolle.model_rolle_id,
                                                                   userHatUserID_id=instance,
                                                                   deleted=False,
                                                                   transfered=True)
                        for af in rolle.applied_afs.all():
                            cpy_af = TblAppliedAf.objects.create(model_af_id=af.model_af_id,
                                                                 applied_rolle_id=cpy_rolle,
                                                                 userHatUserID_id=instance,
                                                                 deleted=False,
                                                                 transfered=False)
                            for gf in af.applied_gfs.all():
                                cpy_gf = TblAppliedGf.objects.create(model_gf_id=gf.model_gf_id,
                                                                     applied_rolle_id=cpy_rolle,
                                                                     applied_af_id=cpy_af,
                                                                     userHatUserID_id=instance,
                                                                     deleted=False,
                                                                     transfered=False)
                                for tf in gf.applied_tfs.all():
                                    cpy_tf = TblAppliedTf.objects.create(model_tf_id=tf.model_tf_id,
                                                                         applied_rolle_id=cpy_rolle,
                                                                         applied_af_id=cpy_af,
                                                                         applied_gf_id=cpy_gf,
                                                                         userHatUserID_id=instance,
                                                                         deleted=False,
                                                                         transfered=False)
                                    cpy_gf.applied_tfs.add(cpy_tf.id)
                                cpy_af.applied_gfs.add(cpy_gf.id)
                            cpy_rolle.applied_afs.add(cpy_af.id)
                        instance.transfer_list.add(cpy_rolle.id)
                        return instance
                    else:
                        for applied_af in rolle.applied_afs.all():
                            af_found = False
                            for trans_af in trans_rolle.applied_afs.all():
                                if applied_af.model_af_id.id == trans_af.model_af_id.id:
                                    af_found = True
                                    break
                            if not af_found:
                                cpy_af = TblAppliedAf.objects.create(model_af_id=applied_af.model_af_id,
                                                                     applied_rolle_id=trans_rolle,
                                                                     userHatUserID_id=instance,
                                                                     deleted=False,
                                                                     transfered=False)
                                for gf in applied_af.applied_gfs.all():
                                    cpy_gf = TblAppliedGf.objects.create(model_gf_id=gf.model_gf_id,
                                                                         applied_rolle_id=trans_rolle,
                                                                         applied_af_id=cpy_af,
                                                                         userHatUserID_id=instance,
                                    ...
                                    for tf in gf.applied_tfs.all():
                                        ...
                                        cpy_gf.applied_tfs.add(cpy_tf.id)
                                    cpy_af.applied_gfs.add(cpy_gf.id)
                                trans_rolle.applied_afs.add(cpy_af.id)
                            else:
                                for applied_gf in applied_af.applied_gfs.all():
                                    gf_found = False
                                    for trans_gf in trans_af.applied_gfs.all():
                                        if applied_gf.model_gf_id.id == trans_gf.model_gf_id.id:
                                            gf_found = True
                                            break
                                    if not gf_found:
                                        cpy_gf = TblAppliedGf.objects.create(model_gf_id=applied_gf.model_gf_id,
                                                                             applied_rolle_id=trans_rolle,
                                                                             applied_af_id=trans_af,
                                                                             userHatUserID_id=instance,
                                                                             deleted=False,
                                                                             transfered=False)
                                        for tf in applied_gf.applied_tfs.all():
                                            cpy_tf = TblAppliedTf.objects.create(model_tf_id=tf.model_tf_id,
                                                                                 applied_rolle_id=trans_rolle,
                                                                                 applied_af_id=trans_af,
                                                                                 applied_gf_id=cpy_gf,
                                                                                 ...
                                                                                 transfered=False)
                                            cpy_gf.applied_tfs.add(cpy_tf.id)
                                        trans_af.applied_gfs.add(cpy_gf.id)
                                    else:
                                        for applied_tf in applied_gf.applied_tfs.all():
                                            tf_found = False
                                            for trans_tf in trans_gf.applied_tfs.all():
                                                if applied_tf.model_tf_id.id == trans_tf.motrans_tf_id.id:
                                                    tf_found = True
                                                    break
                                            if not tf_found:
                                                cpy_tf = TblAppliedTf.objects.create(model_tf_id=applied_tf.model_tf_id,
                                                                                     applied_rolle_id=trans_rolle,
                                                                                     applied_af_id=trans_af,
                                                                                     applied_gf_id=trans_gf,
                                                                                     userHatUserID_id=instance,
                                                                                     deleted=False,
                                                                                     transfered=False)
                                                trans_gf.applied_tfs.add(cpy_tf.id)
                    return instance
                                    ...
                                    for applied_gf in applied_af.applied_gfs.all():
                                        ...
                                        if not gf_found:
                                            ...
                                            for applied_tf in applied_gf.applied_tfs.all():
                                                tf_found = False
                                                for trans_tf in trans_gf.applied_tfs.all():
                                                    if applied_tf.model_tf_id.id == trans_tf.model_tf_id.id:
                                                        tf_found = True
                                                        break
                                                if not tf_found:
                                                    cpy_tf = TblAppliedTf.objects.create(
                                                        model_tf_id=applied_tf.model_tf_id,
                                                        applied_rolle_id=trans_rolle,
                                                        applied_af_id=trans_af,
                                                        applied_gf_id=trans_gf,
                                                        userHatUserID_id=instance,
                                        ...
                                        if applied_tf.model_tf_id.tf == data['right_name'].lower():
                                            ...
                                            for trans_rolle in instance.transfer_list.all():
                                                if rolle.model_rolle_id.rollenid == trans_rolle.model_rolle_id.rollenid:
                                                    ...
                                                    break
                                            if not rolle_found:
                                                cpy_rolle = TblAppliedRolle.objects.create(
                                                    model_rolle_id=rolle.model_rolle_id,
                                                    userHatUserID_id=instance,
                                                    deleted=False,
                                                    transfered=False)
                                                cpy_af = TblAppliedAf.objects.create(
                                                    model_af_id=applied_af.model_af_id,
                                                    applied_rolle_id=cpy_rolle,
                                                    userHatUserID_id=instance,
                                                    deleted=False,
                                                    transfered=False)
                                                cpy_gf = TblAppliedGf.objects.create(
                                                ...
                                                af_found = False
                                                for trans_af in trans_rolle.applied_afs.all():
                                                    if trans_af.model_af_id.id == applied_af.model_af_id.id:
                                                        af_found = True
                                                        break
                                                if not af_found:
                                                    cpy_af = TblAppliedAf.objects.create(
                                                    ...
                                                    gf_found = False
                                                    for trans_gf in trans_af.applied_gfs.all():
                                                        if trans_gf.model_gf_id.id == applied_gf.model_gf_id.id:
                                                            gf_found = True
                                                            break
                                                    if not gf_found:
                                                        cpy_gf = TblAppliedGf.objects.create(
    ...
    def remove_from_transfer_list(self, instance, data):
        ...
        if data['right_type'] == 'role' or data['right_type'] == 'rolle':
            for trans_rolle in instance.transfer_list.all():
                if trans_rolle.model_rolle_id.rollenname == data['right_name']:
                    for trans_af in trans_rolle.applied_afs.all():
                        for trans_gf in trans_af.applied_gfs.all():
                            for trans_tf in trans_gf.applied_tfs.all():
                                trans_gf.applied_tfs.remove(trans_tf)
                                trans_tf.delete()
                            trans_af.applied_gfs.remove(trans_gf)
                            trans_gf.delete()
                        trans_rolle.applied_afs.remove(trans_af)
                        trans_af.delete()
                    instance.transfer_list.remove(trans_rolle)
                    trans_rolle.delete()
                    return instance
        elif data['right_type'] == 'af':
            for trans_rolle in instance.transfer_list.all():
                if trans_rolle.model_rolle_id.rollenname == data['parent']:
                    for trans_af in trans_rolle.applied_afs.all():
                        if trans_af.model_af_id.af_name == data['right_name']:
                            for trans_gf in trans_af.applied_gfs.all():
                                for trans_tf in trans_gf.applied_tfs.all():
                                    trans_gf.applied_tfs.remove(trans_tf)
                                    trans_tf.delete()
                            ...
                            trans_rolle.applied_afs.remove(trans_af)
                            trans_af.delete()
                            if not trans_rolle.applied_afs.all():
                                instance.transfer_list.remove(trans_rolle)
                                trans_rolle.delete()
                            return instance
        elif data['right_type'] == 'gf':
            for trans_rolle in instance.transfer_list.all():
                if trans_rolle.model_rolle_id.rollenname == data['grandparent']:
                    for trans_af in trans_rolle.applied_afs.all():
                        if trans_af.model_af_id.af_name == data['parent']:
                            for trans_gf in trans_af.applied_gfs.all():
                                if trans_gf.model_gf_id.name_gf_neu == data['right_name']:
                                    for trans_tf in trans_gf.applied_tfs.all():
                                        trans_gf.applied_tfs.remove(trans_tf)
                                        trans_tf.delete()
                                    trans_af.applied_gfs.remove(trans_gf)
                                    trans_gf.delete()
                                    if not trans_af.applied_gfs.all():
                                        trans_rolle.applied_afs.remove(trans_af)
                                        trans_af.delete()
                                    if not trans_rolle.applied_afs.all():
                                        instance.transfer_list.remove(trans_rolle)
            ...
            for trans_rolle in instance.transfer_list.all():
                if trans_rolle.model_rolle_id.rollenname == data['greatgrandparent']:
                    for trans_af in trans_rolle.applied_afs.all():
                        if trans_af.model_af_id.af_name == data['grandparent']:
                            for trans_gf in trans_af.applied_gfs.all():
                                    ...
                                    for del_tf in del_gf.applied_tfs.all():
                                        if del_tf.model_tf_id.tf == data['right_name'].lower():
                                            ...
                                            if not rolle_found:
                                                ...
                                                helper_rolle.applied_afs.add(helper_af.id)
                                                instance.rollen.add(helper_rolle.id)
                                                del_gf.applied_tfs.remove(del_tf)
                    ...
                    for applied_af in rolle.applied_afs.all():
                        if applied_af.model_af_id.af_name == data['right_name']:
                            ...
                            if not rolle_found:
                                ...
                                rolle.applied_afs.remove(applied_af)
                                if not rolle.applied_afs.all():
                                    instance.rollen.remove(rolle)
                                return instance
                            else:
                            ...
                            for applied_gf in applied_af.applied_gfs.all():
                                if applied_gf.model_gf_id.name_gf_neu == data['right_name']:
                                    ...
                                    if not rolle_found:
                                        ...
                                        helper_rolle.applied_afs.add(helper_af.id)
                                        instance.delete_list.add(helper_rolle.id)
                                        applied_af.applied_gfs.remove(applied_gf)
                                    ...
                                    for applied_tf in applied_gf.applied_tfs.all():
                                        if applied_tf.model_tf_id.tf == data['right_name'].lower():
                                            ...
                                            if not rolle_found:
                                                ...
                                                helper_rolle.applied_afs.add(helper_af.id)
                                                instance.delete_list.add(helper_rolle.id)
                                                applied_gf.applied_tfs.remove(applied_tf)
...
class TblAppliedGfSerializer(serializers.HyperlinkedModelSerializer):
    class Meta:
        ...
        fields = ('url', 'id', 'model_gf_id', 'userHatUserID_id', 'applied_rolle_id',
                  'applied_af_id', 'applied_tfs', 'deleted', 'transfered', 'requested')
...
class TblAppliedAfSerializer(serializers.HyperlinkedModelSerializer):
    class Meta:
        ...
        fields = ('url', 'id', 'model_af_id', 'userHatUserID_id', 'applied_rolle_id',
                  'applied_gfs', 'deleted', 'transfered', 'requested')
...
class TblAppliedRolleSerializer(serializers.HyperlinkedModelSerializer):
    class Meta:
        ...
        fields = ('url', 'id', 'model_rolle_id', 'userHatUserID_id', 'applied_afs', 'deleted', 'transfered', 'requested')
...
class FullRightsTblAppliedTfSerializer(TblAppliedTfSerializer):
    class Meta:
        model = TblAppliedTf
        fields = ('url', 'id', 'model_tf_id', 'deleted',
...
class FullRightsTblAppliedGfSerializer(TblAppliedGfSerializer):
    class Meta:
        model = TblAppliedGf
        fields = ('url', 'id', 'model_gf_id', 'applied_tfs', 'deleted',
                  'transfered', 'requested')  # ,'userHatUserID_id','applied_rolle_id','applied_af_id'
    ...
    applied_af_id = serializers.HyperlinkedRelatedField(read_only=True,view_name='myRDBNS:appliedaf-detail')
    '''
    applied_tfs = FullRightsTblAppliedTfSerializer(many=True)
...
class FullRightsTblAppliedAfSerializer(TblAppliedAfSerializer):
    class Meta:
        model = TblAppliedAf
        fields = (
        'url', 'id', 'model_af_id', 'applied_gfs', 'deleted', 'transfered', 'requested')  # ,'userHatUserID_id','applied_rolle_id'
    ...
    applied_rolle_id = serializers.HyperlinkedRelatedField(read_only=True,view_name='myRDBNS:appliedrole-detail')
    '''
    applied_gfs = FullRightsTblAppliedGfSerializer(many=True)


class FullRightsTblAppliedRolleSerializer(TblAppliedRolleSerializer):
    class Meta:
        model = TblAppliedRolle
        fields = ('url', 'id', 'model_rolle_id', 'applied_afs', 'deleted', 'transfered', 'requested')  # , 'userHatUserID_id',
    ...
    userHatUserID_id = serializers.HyperlinkedRelatedField(read_only=True,
                                                           view_name='myRDBNS:userhatuseridundname-detail')
    '''
    applied_afs = FullRightsTblAppliedAfSerializer(many=True)
...
class FullRightsUserHatTblUserIDundNameSerializer(UserHatTblUserIDundNameSerializer):
    ...
    user_name = serializers.HyperlinkedRelatedField(read_only=True, view_name='myRDBNS:user-detail')
    userid_name_id = serializers.HyperlinkedRelatedField(read_only=True, view_name='myRDBNS:useridundname-detail')
    rollen = FullRightsTblAppliedRolleSerializer(many=True)
    delete_list = FullRightsTblAppliedRolleSerializer(many=True)
    transfer_list = FullRightsTblAppliedRolleSerializer(many=True)
    my_requests = serializers.HyperlinkedRelatedField(many=True, read_only=True, view_name='myRDBNS:changerequests-detail')
